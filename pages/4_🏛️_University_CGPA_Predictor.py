...
st.set_page_config(page_title="Predict — University", page_icon="🏫", layout="wide")
load_css()
...
render_navbar(active="Predict University")

st.title("🏫 CGPA Prediction — University Students")
st.markdown(
    "Answer the questions below to get an estimated **CGPA band**, based on "
    "patterns learned from 592 University student survey responses."
)
...
with st.form("university_prediction_form"):

    st.subheader("👤 Student Information")
    with st.container(border=True):
        c1, c2, c3 = st.columns(3)
        age = c1.selectbox("Age", ["18 - 21", "21 - 25", "25 - 28", "28+"])
        gender = c2.selectbox("Gender", ["Male", "Female"])
        level = c3.selectbox("Level of Study", sorted(df_university[col["level_col"]].dropna().unique().tolist()))
    ...
    with st.container(border=True):
        ...
        purpose = c3.selectbox("Primary purpose", sorted(df_university[col["purpose_col"]].dropna().unique().tolist()))

        c1, c2 = st.columns(2)
        platform = c1.selectbox("Most-used platform", sorted(df_university[col["platform_col"]].dropna().unique().tolist()))
    ...
    st.markdown("")
    submitted = st.form_submit_button("🔮 Predict My CGPA Band", use_container_width=True)
...
if submitted:
    ...
    predicted_band, confidence = predict(raw_input, artifacts, model_type="university")
    ...
    with st.container(border=True):
        ...
        with c1:
            st.markdown("**Predicted CGPA Band**")
            st.markdown(f"## 🎯 {predicted_band}")
            top_confidence = max(confidence.values())
            st.caption(f"Model confidence: {top_confidence:.0%}")
        with c2:
            confidence_series = pd.Series(confidence).reindex(artifacts["university_target_order"])
            fig, ax = plt.subplots(figsize=(7, 3.2))
            colors = ["#EA580C" if band == predicted_band else "#FED7AA" for band in confidence_series.index]
    ...
    image_buf = generate_result_image(predicted_band, confidence_series, "Predicted CGPA Band", "#EA580C")

    st.download_button(
        label="⬇️ Download My Result",
        data=image_buf,
        file_name="my_cgpa_prediction.png",
        mime="image/png",
    ...
    recommendations = get_recommendations(raw_input, artifacts, fi_data, model_type="university")
    ...
    comparisons = get_percentile_comparisons(raw_input, eda_data, model_type="university")
